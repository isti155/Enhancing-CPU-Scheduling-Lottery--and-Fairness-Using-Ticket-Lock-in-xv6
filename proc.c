...
static unsigned
krand(void) {
  /* Linear congruential generator (64-bit state), return 32-bit value */
...
scheduler(void)
{
  ...
  for(;;){
    ...
    // 1) Compute total effective tickets
    int total = 0;

    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state == RUNNABLE){

        // Merge reward tickets
    ...
    if(total == 0){
      // No runnable process → continue round-robin
      release(&ptable.lock);
      continue;
    }

    // 2) Choose the winning ticket
